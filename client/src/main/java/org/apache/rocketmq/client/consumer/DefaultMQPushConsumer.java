...
/**
 * In most scenarios, this is the mostly recommended class to consume messages. </p>
 *
 * Technically speaking, this push client is virtually a wrapper of the underlying pull service. Specifically, on
 * arrival of messages pulled from brokers, it roughly invokes the registered callback handler to feed the messages.
 * </p>
 *
 * See quickstart/Consumer in the example module for a typical usage. </p>
 *
 * <p>
...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
	 ...
	 * load balance. It's required and needs to be globally unique. </p>
	 *
	 ...
	 * <ul>
	 * <li>
	 * <code>CONSUME_FROM_LAST_OFFSET</code>: consumer clients pick up where it stopped previously. If it were a newly
	 * booting up consumer client, according aging of the consumer group, there are two cases:
	 * <ol>
	 * <li>
	 * if the consumer group is created so recently that the earliest message being subscribed has yet expired, which
	 * means the consumer group represents a lately launched business, consuming will start from the very beginning;</li>
	 * <li>
	 * if the earliest message being subscribed has expired, consuming will start from the latest messages, meaning
	 * messages born prior to the booting timestamp would be ignored.</li>
	 * </ol>
	 * </li>
	 * <li>
	 * <code>CONSUME_FROM_FIRST_OFFSET</code>: Consumer client will start from earliest messages available.</li>
	 * <li>
	 * <code>CONSUME_FROM_TIMESTAMP</code>: Consumer client will start from specified timestamp, which means messages
	 * born prior to {@link #consumeTimestamp} will be ignored</li>
	 * </ul>
	 */
	...
	/**
	 * Max re-consume times. -1 means 16 times. </p>
	 *
	...
	public DefaultMQPushConsumer() {
		this(MixAll.DEFAULT_CONSUMER_GROUP, null, new AllocateMessageQueueConsistentHash());
	}
	...
	public DefaultMQPushConsumer(RPCHook rpcHook) {
		this(MixAll.DEFAULT_CONSUMER_GROUP, rpcHook, new AllocateMessageQueueConsistentHash());
	}
	...
	public DefaultMQPushConsumer(final String consumerGroup) {
		this(consumerGroup, null, new AllocateMessageQueueConsistentHash());
	}
